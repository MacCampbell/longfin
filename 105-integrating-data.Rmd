...
srun -p high -t 14:00:00 --mem=16G --nodes=1 $HOME/angsd/angsd -P 12 \
  ...
  -doGLF 2 -doMajorMinor 1 -doMaf 2 -SNP_pval 1e-6 -minMapQ 10 -minQ 20 -minMaf 0.05 \
   -r lg01  \
   -minInd 195 \
  -out $HOME/longfin/outputs/105/195-beagle > outputs/105/beagle-contigs.out 2> outputs/105/beagle-contigs.err &
  

python $HOME/pcangsd/pcangsd.py -beagle outputs/105/195-beagle.beagle.gz \
  -o outputs/105/195-pca -threads 10
```
...
-rf $HOME/delta-smelt/metadata/large-contigs.txt
```{r}
cov<-read_delim("outputs/105/283-pca.cov", col_names=FALSE, delim=" ") %>% as.matrix()
```

```{r}
m283<-c
meta<-m283
meta$Location<-factor(meta$Location, levels=c("YBAK","SKNA","PTLC","HRLC","FRAS","LWSH","COLR",
...
pcs
ggsave("outputs/105/combined-pca-283.jpeg", width=12)
```
What's with those SFBY fish

```{r}
SFBY<-sub12 %>% filter(val_x< -0.3)

ggplot(SFBY, aes(x = val_x, y = val_y, color=Location))+
  geom_point(size = 2, alpha=0.75) +
  scale_fill_discrete(na.value = "white") + 
  geom_label_repel(aes(x=val_x, y=val_y, label=paste0(sample,"-",Dedup)))+
  theme_bw()+
  theme(panel.grid=element_blank())+
  xlab(paste("PC1", " ", round((100*var[1]),2), "%", sep = ""))+
  ylab(paste("PC2", " ", round((100*var[2]),2), "%", sep = ""))+
...
```{sh, eval=FALSE}
srun -p high -t 01:00:00 --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/105/283-beagle.beagle.gz -K 2 -o outputs/105/283-K2
srun -p high -t 01:00:00 --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/105/283-beagle.beagle.gz -K 3 -o outputs/101/283-K3
srun -p high -t 01:00:00 --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/105/283-beagle.beagle.gz -K 4 -o outputs/101/283-K4
srun -p high -t 01:00:00 --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/105/283-beagle.beagle.gz -K 5 -o outputs/101/283-K5

```


```{r}
q2<-read_delim("outputs/105/283-K2.qopt", delim=" ", col_names=FALSE) %>% select(X1, X2) %>%
  rename(Q1=X1, Q2=X2)
