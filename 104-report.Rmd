...
v<-ggtree(u) %<+% m216 
data<-v$data
ddf<-data  %>% left_join(names, by=c("label"="Sample")) %>% filter(isTip==TRUE) %>% select(label, Species) %>%  replace(is.na(.), "Larval Smelt")
ddf$Species<-factor(ddf$Species, levels=c("Larval Smelt", "Longfin Smelt", "Night Smelt", "Eulachon", "Whitebait Smelt", "Wakasagi", "Surf Smelt", "Delta Smelt" ))
...
srun -p high -t 14:00:00 --mem=16G --nodes=1 $HOME/angsd/angsd -P 36 \
  -ref /home/maccamp/genomes/hypomesus-20210204/Hyp_tra_F_20210204.fa \
  -bam $HOME/longfin/bamlists/141.bamlist -GL 1 \
  -doGLF 2 -doMajorMinor 1 -doMaf 2 -SNP_pval 1e-3 -minMapQ 10 -minQ 20 -minMaf 0.05 \
  -rf $HOME/delta-smelt/metadata/large-contigs.txt  -minInd 71 \
  -out $HOME/longfin/outputs/105/141> outputs/104/beagle-141.out 2> outputs/104/beagle-141.err &
