...
```{r}
##Phylometa
phylometa<-ddf %>% left_join(m216, by=c("label"="Sample")) %>% group_by(Category)  %>% 
  summarize(`Sample Size`=n(), `Average Read Count`=mean(Dedup), 
                                          `Median Read Count`=median(Dedup))
write_csv(phylometa, file="meta/phylo-meta.csv") 
```
